...
public class Utils {
   ...
   * <p>
   * Example: <code>"[RA]&lt;sp/&gt;SvnthSyn"</code> =>
   * <code>"[RA]SvnthSyn"</code>
   * </p>
   *
   ...
   * </p>
   * <ul>
   * <li>The output will start with an upper-cased letter if input starts with a
   * letter (regardless of word capitalization).
   * <li>Word boundaries are defined by underscore characters
   * (<code>'_'</code>). Underscores are replaced with spaces.
   * <li>First letters of words will be capitalized based on the
   * <code>capitalizeWords</code> parameter.

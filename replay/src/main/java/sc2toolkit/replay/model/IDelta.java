...
public interface IDelta extends IStructView {
   ...
   * <p>
   * <p>
   * If the second value of the returned pair ({@link IPair#getValue2()}) is not
   * the string <code>"None"</code>, then the first value of the returned pair (
   * {@link IPair#getValue1()}) may have the following values:
   * </p>
   * <ul>
   * <li><code>"ZeroIndices"</code> - in this case the second value is an
   * {@link Integer} array (<code>Integer[]</code>) listing the retained indices
   * from the current selection, units of unlisted index values are removed</li>
   * <li><code>"OneIndices"</code> - second value is an {@link Integer} array
   * (<code>Integer[]</code>) listing the indices that are removed</li>
   * <li><code>"Mask"</code> - second value will be a {@link IBitArray}
   * specifying a deselection map: units at indices where bit is set are
   * removed, deselection count is the number of <code>1</code>s
   * ({@link IBitArray#getOnesCount()})</li>
   * </ul>
   *

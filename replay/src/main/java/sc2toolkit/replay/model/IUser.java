...
public interface IUser {
   ...
   * In ladder games <code>playerId = playerIdx + 1</code> but it can deviate in
   * custom games.
   *
  ...
  /**
   * Returns the {@link IPlayer} object from the
   * {@link IDetails#getPlayerList()}, if any.
   *
   * @return the {@link IPlayer} object from the
   *         {@link IDetails#getPlayerList()}, if any
   */
  IPlayer getPlayer();

  /**
   * Returns the player user index after optional reordering (e.g. favored
   * player list) in {@link IRepProcessor#getPlayerUsers()}.
   *
  ...
  /**
   * Returns the user index (index in the {@link IRepProcessor#getUsers()}
   * array.
   *
   ...
   * Additionally: time range: initial time is excluded based on setting, and
   * last loop is the loop of the last cmd game event of the user; and actions
   * are all game events within this time range except camera update game
   * events.
   * </p>
   *
   ...
   * Additionally: time range: initial time is excluded based on setting, and
   * last loop is the loop of the last cmd game event of the user; and actions
   * are all camera update events which move the screen by a distance larger
   * than 15.
   * </p>
   *
   ...
   * Additionally: samples are taken up to the loop of the last cmd game event
   * of the user.
   * </p>
   *
  ...
  /**
   * Returns the direction of the start location of the player from the center
   * of the map in the range of 1..12 as an hour value.
   *
   * @return the direction of the start location of the player from the center
   *         of the map in the range of 1..12 as an hour value
   */
  int getStartDirection();

  /**
   * Returns the supply-capped percent (ratio of supply-capped time up until the
   * last cmd game event and the time of the last cmd game event of the user).
   *
   * @return the supply-capped percent (ratio of supply-capped time up until the
   *         last cmd game event and the time of the last cmd game event of the
   *         user)
   */
  double getSupplyCappedPercent();

  /**
   * Returns the start location on the map, matched by the first camera update
   * event (which has a target point).
   *
   * @return the start location on the map, matched by the first camera update
   *         event (which has a target point)
   */
   ...
   * <p>
   * The returned color originates from {@link ISlot#getPlayerColor()} in
   * non-archon games, and in case of {@link IFormat#ARCHON} it returns the
   * tandem leader's color.
   * </p>
   *

...
public interface IPlayer extends IStructView {
   ...
   * @see #getResult()
   */
  Result getDeducedResult();

  /**
   * Returns the match result of the player, either the one recorded in the
   * replay ({@link #getRecordedResult()}, or the deduced result (
  ...
  /**
   * Tells if the match result returned by {@link #getResult()} is originating
   * from the replay or is a deduced result determined by an algorithm.
   *
   * @return true if the match result originates from the replay; false if it is
   *         a deduced result determined by an algorithm
   */
  boolean isResultDeduced();

  /**
   * Returns the team id, not always accurate! Use {@link ISlot#getTeamId()}
   * instead!
   *
   * @return the team id, not always accurate! Use {@link ISlot#getTeamId()}
   *         instead!
   *
   * @see ISlot#getTeamId()
   */

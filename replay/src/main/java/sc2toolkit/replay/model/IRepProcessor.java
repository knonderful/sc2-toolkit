...
public interface IRepProcessor {
  ...
  /**
   * Returns the initial time to exclude from per-minute calculations in
   * game-time seconds.
   *
   * @return the initial time to exclude from per-minute calculations in
   *         game-time seconds
   */
   ...
   *
   * @return true if detected format is to be overridden based on matchup; false
   *         otherwise
   */
   ...
   * missing.
   *
   * @return true if largest remaining team is to be declared winner if result
   *         info is missing; false otherwise
   */
  boolean isLargestRemainingTeamWins();

  /**
   * Returns the unit tag transformation strategy that is used when unit tags
   * are displayed.
   *
   * @return the unit tag transformation strategy
   */
  TagTransformation getTagTransformation();
  ...
  /**
   * Returns the game speed to be used to convert between game-time and
   * real-time.
   * <p>
   * <p>
   * Game speeds instances support converting time values between game time and
   * real time. This game speed is used by {@link #convertToRealTime(long)} and
   * {@link #convertToRealTime(long)}.
   * </p>
   * <p>
   * <p>
   * Note: since Legacy of the Void expansion is using real-time,
   * {@link IGameSpeed#NORMAL} is returned always in case of LotV replays.
   * </p>
   *
   * @return the game speed to be used to convert between game-time and
   *         real-time
   *
   * @see #getConverterGameSpeed()
   *
  ...
  /**
   * Returns the users (players, observers etc.). Contains no <code>null</code>
   * values.
   *
  ...
  /**
   * Returns the users indexed by user id, might contain <code>null</code>
   * values.
   *
  ...
  /**
   * Returns the users indexed by player id. First element is <code>null</code>
   * (player id is 1-based)
   *
   ...
   * Contains no <code>null</code> values.
   *
  ...
  /**
   * Calculates a Per Minute value as a <code>int</code> type (e.g. Actions Per
   * Minute, Screens Per minute).
   *
   ...
   * @return the calculated Per Minute value (e.g. Actions Per Minute, Screens
   *         Per minute)
   *
  ...
  /**
   * Calculates a Per Minute value as a <code>double</code> type (e.g. Actions
   * Per Minute, Screens Per minute).
   *
   ...
   *
   * @return the guessed format; {@link IFormat#CUSTOM} if format cannot be
   *         guessed
   */
  Format guessFormat();
   ...
   * <p>
   * A game is Archon mode if the {@link IFormat} (as returned by
   * {@link #getFormat()} is {@link IFormat#ARCHON}.
   * </p>
   *
  ...
  /**
   * This method converts game-time to real-time based on the user's preference
   * <i>at construction time</i> (the value is cached and used so no
   * inconsistent time values are produced if the setting is modified later).
   *
   * @param gameMs game time to be converted
   * @return the same time if real time is not enabled; else the game time
  ...
  /**
   * This method converts real-time to game time based on the user's preference
   * <i>at construction time</i> (the value is cached and used so no
   * inconsistent time values are produced if the setting is modified later).
   * <p>
   * <p>
   * On a game speed conversion from game-time to real-time is multiplication by
   * a constant, converting back means to multiply with the reciprocal of the
   * constant:<br>
   * realMs = gameMs * c<br>
   * gameMs = realMs / c<br>
   * <br>
   * Converting values in which time is a divisor must be reversed, for
   * example:<br>
   * APM = actions / time<br>
   * game APM = actions / game time<br>
   * real APM = actions / real time = actions / ( game time * c) = ( actions /
   * game time ) / c = game APM / c<br>
   * </p>
   * <p>
   * <p>
   * <b>So basically this method can be used to convert APM values from
   * game-time to real-time!</b>
   * </p>
   *
   * @param realMs real time to be converted
   * @return the same time if real time is not enabled; else the real time
   ...
   * <p>
   * <p>
   * This method also converts game-time to real-time based on the user's
   * preference <i>at construction time</i> (the value is cached and used so no
   * inconsistent time values are produced if the setting is modified later).
   * </p>
   *
  ...
  /**
   * Returns a comma separated list of player names whose result is the
   * specified optional result.
   *
   * @param result optional result filter; if specified, only players having
   *               this result will be included
   * @return a comma separated list of player names
   */
  String getPlayersStringOfResult(Result result);
   ...
   * <p>
   * <p>
   * The weights are the times used to calculate a player APM value. Only human
   ...
   * <p>
   * <p>
   * The weights are the times used to calculate a player SPM value. Only human
   * players are included.
   * </p>
   *
   ...
   * <p>
   * <p>
   * The algorithm rounds up, for example if there are 2 players having 2
   * different subsequent leagues, then the average will be the higher league.
   * </p>
   *
   ...
   *
   * @return the number of chat messages or -1 if the message events is not
   *         parsed
   */

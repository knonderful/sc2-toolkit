...
public interface ITargetUnit extends IStructView {
   ...
   * In old versions (below 1.4) there is only "snapshotPlayerId" instead of
   * "snapshotControlPlayerId".
   */
  String F_SNAPSHOT_PLAYER_ID = "snapshotPlayerId";

  /**
   * Target flags just notes the visibility of the targeted unit (hidden,
   * cloaked, fogged, visible, conjoined).
   *
  ...
  /**
   * Timer seems to be related to whether the unit was recently visible, to do
   * with losing targets (probably when they disappear under the fog?).
   *
   * @return the timer
   */
  Integer getTimer();

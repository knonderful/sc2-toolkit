...
public abstract class BitPackedBuffer {
  ...
  /**
   * Bit masks, masking the bits of a byte for the bits count specified by the
   * index.
   */
   ...
   * Bit masks, masking the bits of a byte for the bits count specified by the
   * index.<br>
   * Values are <code>long</code>s so when masking, result will be automatically
   * a <code>long</code>.
   */
  ...
  /**
   * Clears the current byte, drops unread bits from it, so the next read will
   * happen from the next byte. This is to byte align the bit input stream.
   */
   ...
   * <p>
   * The method also skips bits left in the cache (by calling
   * {@link #byte_align()} first).
   * </p>
   *
  ...
  /**
   * Reads the specified amount of bits using big endian byte order and returns
   * it as a <code>int</code>.
   *
   * @param n number of bits to read; cannot be greater than 32
   * @return the read bits as an int
   */
  ...
  /**
   * Reads the specified amount of bits using little endian byte order and
   * returns it as a <code>int</code>.
   *
   * @param n number of bits to read; cannot be greater than 32
   * @return the read bits as an int
   */
  ...
  /**
   * Reads the specified amount of bits using little endian byte order and
   * returns it as a <code>long</code>.
   *
   * @param n number of bits to read; cannot be greater than 64
   * @return the read bits as an int
   */
   ...
   * <p>
   * <p>
   * If <code>typeid</code> denotes a structure, a structure of initial capacity
   * defined by the parameters of the {@link TypeInfo} will be allocated. If the
   * returned structure is manually extended (new entries are added), the
   * structure type most likely needs to internally reallocate to accommodate
   * the extra entries. This can be avoided if the number of extra entries
   * (which will be added in the future) is passed as the
   * <code>extraEntriesCount</code> parameter, which will be used to allocate a
   * structure with a bigger initial capacity.
   * </p>
   *
   * @param typeid            type id of the object to read
   * @param extraEntriesCount optional number of extra structure entries to
   *                          allocate (for future manual additions)
   * @return the read object
   */
   ...
   * This is an optimized version of reading multiple objects of the same type,
   * type-related checks and conversions are only performed once!<br>
   * Aids the creation of an optimized implementation for the
   * {@link #_array(IntBounds, int)}.
   * <p>
   * <p>
   * While return type is specified to be <code>Object[]</code>, the dynamic
   * type is a more concrete array type, for example if the type method
   * specified by <code>typeid</code> is {@link TypeMethod#INT}, the returned
   * type will be <code>Integer[]</code>.
   * </p>
   *
  ...
  /**
   * Reads a <a href="http://en.wikipedia.org/wiki/FourCC">FourCC</a> (4
   * characters).
   *
   ...
   * <p>
   * First reads a boolean, and if that is <code>true</code>, then reads an
   * object and returns it; if it is <code>false</code>, <code>null</code> is
   * returned.
   * </p>
   *
   * @param typeid type id of the object to read

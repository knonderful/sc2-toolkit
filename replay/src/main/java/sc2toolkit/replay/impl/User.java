...
import sc2toolkit.replay.model.IUser;
import sc2toolkit.replay.model.PlayerColor;
...
public class User implements IUser {
  ...
  /**
   * Player index in the array returned by {@link Details#getPlayerList()}, in
   * ladder games <code>{@link #playerId} = {@link #playerIdx} + 1</code> but in
   * custom games it can deviate.
   */
  public final int playerIdx;

  /**
   * Player Id, in ladder games <code>{@link #playerId} = {@link #playerIdx} +
   * 1</code> but in custom games it can deviate.
   */
  ...
  /**
   * Player user index after optional reordering (e.g. favored player list) in
   * {@link RepProcessor#playerUsers}.
   */
  ...
  /**
   * Direction of the start location of the player from the center of the map in
   * the range of 1..12 as an hour value.
   */
  public int startDirection;

  /**
   * Supply-capped percent (ratio of supply-capped time up until the last cmd
   * game event and the time of the last cmd game event of the user).
   */
  public double supplyCappedPercent;

  // Derived data from map objects
  /**
   * Start location on the map, matched by the first camera update event (which
   * has a target point).
   */

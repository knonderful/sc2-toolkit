...
import sc2toolkit.replay.model.Region;
...
public class Toon extends StructView implements IToon {
  ...
  /**
   * Optional player name extracted from the string representation (see
   * {@link #Toon(String)}).
   */
   ...
   *             exactly the form produced by {@link #toString()}
   *
   ...
   * Creates a new {@link Toon}.
   *
   * @param toon            toon string representation to parse and initialize
   *                        from; exactly the form produced by
   *                        {@link #toString()}
   * @param parsePlayerName tells if player name is to be parsed from the toon
   *                        string if present
   *
   * @throws IllegalArgumentException if the specified toon string is invalid
   *
  ...
  /**
   * Produces a string representation in a form that is required by the
   * {@link #Toon(String)} constructor.
   *
   * @see #Toon(String)
   * @see #Toon(String, boolean)
   */

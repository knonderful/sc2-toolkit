...
public enum DurationFormat {
	...
	public String formatDuration( long ms ) {
		switch ( this ) {
			...
			case MS :
				return Env.LANG.formatNumber( ms ) + " ms";
			case SEC :
				return Env.LANG.formatNumber( ms / 1000 ) + " sec";
			case SEC_MS :
				return Env.LANG.formatNumber( ms / 1000.0, 3 ) + " ms";
			case MIN_SEC : {
				final StringBuilder b = new StringBuilder( 8 );
				
				// Min
				if ( ms < 10 * Utils.MS_IN_MIN )
					b.append( '0' );
				b.append( ms / Utils.MS_IN_MIN );
				// Sec
				b.append( ':' );
				ms %= Utils.MS_IN_MIN;
				if ( ms < 10_000 )
			...
			case HOUR_MIN_SEC : {
				...
				// Hour
				if ( ms < 10 * Utils.MS_IN_HOUR )
					b.append( '0' );
				b.append( ms / Utils.MS_IN_HOUR );
				// Min
				b.append( ':' );
				ms %= Utils.MS_IN_HOUR;
				if ( ms < 10 * Utils.MS_IN_MIN )

...
public class SelectionTracker implements ISelectionTracker {
  ...
  /**
   * Selections (list of unit links and unit tags) for the 10 control groups and
   * an extra one for the active selection (active selection is referenced as
  ...
  /**
   * Reference to the active selection (which is the last element of
   * {@link #cgSelections}).
   */
  ...
  /**
   * Processes the specified deselection map on the specified selection. The
   * deselection map is a bit array where the position of bits set to 1 are unit
  ...
  private static void processDeselectionMap(final IBitArray mask, final List< Integer[]> selection) {
    if (mask != null) {
      for (int i = Math.min(selection.size() - 1, mask.getCount() - 1); i >= 0; i--) // Since we're modifying the list, we have to go downward!
      {
        if (mask.getBit(i)) {
          selection.remove(i);
        }

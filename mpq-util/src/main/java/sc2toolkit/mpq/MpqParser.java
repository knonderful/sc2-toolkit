...
import sc2toolkit.mpq.AlgorithmUtil.MpqHashType;
 ...
 * </p>
 * <ul>
 * <li><code>int version:</code> Version of the (attributes) file. Must be 100
 * (0x64)
 * <li><code>int flags:</code> flags telling what is contained in the
...
public class MpqParser implements AutoCloseable {
   ...
   * <code>AlgorithmUtil.hashString( "(hash table)", {@link MpqHashType#FILE_KEY}
   * )</code>
   */
   ...
   * <code>AlgorithmUtil.hashString( "(block table)", {@link MpqHashType#FILE_KEY}
   * );</code>
   */
  ...
  /**
   * Creates a new MpqParser, opens the specified MPQ file and reads the MPQ
   * headers: user data, mpq header, hash tables, block tables.
   * <p>
   * <p>
   * If no exception is thrown, the parser will keep the file of the MPQ archive
   * open. A call to the method <code>close()</code> is required to close the
   * file.
   * </p>
   *
   * @param file MPQ file
   * @throws InvalidMpqArchiveException if the specified file is not a valid MPQ
   *                                    archive
   */
  ...
  /**
   * Creates a new MpqParser, and reads the MPQ headers: user data, mpq header,
   * hash tables, block tables.
   * <p>
   * <p>
   * If no exception is thrown, the parser will keep the data input open. A call
   * to the method <code>close()</code> is required to close the data input.
   * </p>
   *
   * @param mpqDataInput data input to read the content of the MPQ from
   * @throws InvalidMpqArchiveException if the specified input does not produces
   *                                    a valid MPQ archive
   */
  ...
  /**
   * Returns the extracted size of a file in the archive determined by the 3
   * different hash values of the file name (and path).
   *
   ...
   * @return the extracted size of the specified file in the archive; or
   *         <code>-1</code> if the archive does not contain the specified file
   */
   ...
   * @return the content of the specified file; or <code>null</code> if the
   *         archive does not contain the specified file
   * @throws InvalidMpqArchiveException if the storing method of the file is not
   *                                    supported/implemented or some error
   *                                    occurs
   */
  ...
  /**
   * Returns the content of a file taken from the archive determined by the 3
   * different hash values of the file name (and path).
   *
   * @param hash1 {@link MpqHashType#TABLE_OFFSET} hash value of the file name
   * @param hash2 {@link MpqHashType#NAME_A} hash value of the file name
  ...
  /**
   * Reads the specified amount of bytes from the input stream and wraps it into
   * a ByteBuffer.<br>
   * Little endian byte order is set in the returned buffer.<br>
   * If EOF is reached before the full buffer is read, IOException will be
   * thrown.
   *
   * @param size number of bytes to read
   * @return a ByteBuffer wrapping the byte array containing the read bytes
   * @throws IOException if IOException occurs during the read or if EOF is
   *                     reached before <code>size</code> bytes are read
   */

...
public enum MpqContent implements IMpqContent {
   ...
   * @param hash1    {@link MpqHashType#TABLE_OFFSET} hash value of the file
   *                 name
   ...
   * @param name       optional name to use, if missing, the normalized enum
   *                   name will be used (normalized by
   *                   {@link LUtils#constNameToNormal(String, boolean)}
  ...
  public static < T extends Enum< ?> & IMpqContent> String getDisplayName(final String name, final T mpqContent) {
    return "<html>" + (name == null ? Utils.constNameToNormal(mpqContent.name(), true) : name) + " (<code>" + mpqContent.getFileName() + "</code>)";
  }
